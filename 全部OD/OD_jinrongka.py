...
def coords(city):
    ...
    if 'geocodes' not in jd.keys() or len(jd['geocodes']) == 0 or 'location' not in jd['geocodes'][0].keys():
        return 0
    return jd['geocodes'][0]['location']


def oodd1(jinrongka_no,chepai,jingweidu,shua_jinrongka,mycol,backValue,backValue2,backValue3,backValue4):
    a = {}
    bb = {}
    not_pipei_dict = {}
    only_zaowan_oncebycar_dict = {}
    for i in jinrongka_no:
#        i = jinrongka_no[0]
        print(jinrongka_no.index(i))
        shijian = shua_jinrongka[shua_jinrongka['card_no'] == i].loc[:,'riding_time']
        if len(shijian) ==1 :
            continue
        busPlate = shua_jinrongka[shua_jinrongka['card_no'] == i].loc[:,'busPlate']
        lunzi_id = []
    ...
    not_pipei_dict['not_pipei'] = len(jinrongka_no) - len(a)
    '''删除只乘一次车的乘客'''
    bbb = []          
    for i in a.keys():
        if len(a[i]) == 1:
    ...
    for i in temp.keys():
        ...
        if len(station_index1) == 0 or len(station_index2) == 0 :
            a_up_down[i] = {'上车':result1[0],'经纬度_morning':coords(result1[0]),\
                     '下车':result2[0],'经纬度_aft':coords(result2[0]),'概率':\
            ...
            a_up_down[i] = {'上车':result1[0],'经纬度_morning':[code1_jing,code1_wei],\
                     '下车':result2[0],'经纬度_aft':[code2_jing,code2_wei],'概率':\
                 (temp[i][result1[0]]+temp_evening[i][result2[0]])/(p_xiache(temp[i])+p_xiache(temp_evening[i]))}
    '''只有早晚的'''
    for i in temp_neither.keys():
        ...
        if len(station_index3) == 0 or len(station_index4) == 0 :
            a_up_down[i] = {'上车':result3[0],'经纬度_morning':coords(result3[0]),\
                     '下车':result3[1],'经纬度_aft':coords(result3[1]),'概率':\
            ...
            a_up_down[i] = {'上车':result3[0],'经纬度_morning':[code1_jing,code1_wei],\
                     '下车':result3[1],'经纬度_aft':[code2_jing,code2_wei],'概率':\
                 (temp_neither[i][result3[0]]+temp_neither[i][result3[1]])/p_xiache(temp_neither[i])}
...
def multithreading(jinrongka_no,mean,chepai,jingweidu,shua_jinrongka,mycol):
    ...
    threads = []
    # 数据数组
    data = [jinrongka_no[:mean],jinrongka_no[mean:2*mean],jinrongka_no[2*mean:3*mean],jinrongka_no[3*mean:4*mean],jinrongka_no[4*mean:5*mean],jinrongka_no[5*mean:6*mean],\
            jinrongka_no[6*mean:7*mean],jinrongka_no[7*mean:8*mean],jinrongka_no[8*mean:9*mean],jinrongka_no[9*mean:10*mean],jinrongka_no[10*mean:11*mean],jinrongka_no[11*mean:12*mean],\
            jinrongka_no[12*mean:13*mean],jinrongka_no[13*mean:14*mean],jinrongka_no[14*mean:15*mean],jinrongka_no[15*mean:]]
 
    # 在多线程函数中定义四个线程，启动线程，将每个线程添加到多线程的列表中
    # 定义四个线程
    for i in range(len(data)):
        # Thread首字母要大写，被调用的job函数没有括号，只是一个索引，参数在后面
        t1 = td.Thread(target=oodd1, args=(data[i],chepai,jingweidu,shua_jinrongka,mycol,backValue,backValue2,backValue3,backValue4))
        t1.start()  # 开始线程
...
def main(time):
    ...
    myclient = MongoClient('your localhost')#真实
    ...
    '''刷金融卡数据'''
    shua_jinrongka = []
    mycol = mydb['financial_paid_record']
    myquery = {'riding_time':re.compile(time)}
    for x in mycol.find(myquery):
        shua_jinrongka.append(x)
    shua_jinrongka = pd.DataFrame(shua_jinrongka)
    jinrongka_no = list(set(shua_jinrongka['card_no']))
    mean = int(len(jinrongka_no)/16)
    mycol = mydb['stop_arrive_leave']
    
    a_up_down_list,bbb,not_pipei,onlyzaowan = multithreading(jinrongka_no,mean,chepai,jingweidu,shua_jinrongka,mycol)
    a_up_down = {}
...
#            print('aaa')
    ...
    for i in a_up_down.keys():
        mydict_all = {'rate':a_up_down[i]['概率'],'cardId':str(i),'lat_long_morning':a_up_down[i]['经纬度_morning'],\
                      ...
                      'date':time,'cardType':2,'up_direct':a_up_down[i]['方向O'],'down_direct':a_up_down[i]['方向D']}
