# -*- coding: utf-8 -*-
"""
Created on Tue Aug 13 10:19:20 2019
...
def find_direct(location,direct,i):
    ...
    if num1 >= num2:
        ...
        return 1
def oodd1(shua_ka_no,chepai,jingweidu,shua_ka,mycol,backValue,backValue2,backValue3,backValue4):
    a = {}
    bb = {}
    direct = {}
    not_pipei_dict = {}
    ...
    for i in shua_ka_no:
#        i = erweima_no[0]
        print(shua_ka_no.index(i))
        shijian = shua_ka[shua_ka['card_no'] == i].loc[:,'riding_time']
        if len(shijian) ==1:
            continue
        busPlate = shua_ka[shua_ka['card_no'] == i].loc[:,'busPlate']
        lunzi_id = []
...
#            print(uuu)
    ...
    not_pipei_dict['not_pipei'] = len(shua_ka) - len(a)
    '''删除只乘一次车的乘客'''
    bbb = []          
    for i in a.keys():
        if len(a[i]) == 1:
...
def multithreading(shua_ka_no,mean,chepai,jingweidu,shua_ka,mycol):
    ...
    threads = []
    # 数据数组
    data = [shua_ka_no[:mean],shua_ka_no[mean:2*mean],shua_ka_no[2*mean:3*mean],\
            shua_ka_no[3*mean:4*mean],shua_ka_no[4*mean:5*mean],shua_ka_no[5*mean:6*mean],shua_ka_no[6*mean:7*mean],\
            shua_ka_no[7*mean:8*mean],shua_ka_no[8*mean:9*mean],shua_ka_no[9*mean:10*mean],\
            shua_ka_no[10*mean:11*mean],shua_ka_no[11*mean:12*mean],shua_ka_no[12*mean:13*mean],shua_ka_no[13*mean:14*mean],\
            shua_ka_no[14*mean:15*mean],shua_ka_no[15*mean:]]
 
    # 在多线程函数中定义四个线程，启动线程，将每个线程添加到多线程的列表中
    # 定义四个线程
    for i in range(len(data)):
        # Thread首字母要大写，被调用的job函数没有括号，只是一个索引，参数在后面
        t1 = td.Thread(target=oodd1, args=(data[i],chepai,jingweidu,shua_ka,mycol,backValue,backValue2,backValue3,backValue4))
        t1.start()  # 开始线程
...
def main(time):
    ...
    myclient = MongoClient('your locatlhost')#真实
    ...
    '''刷卡数据'''
    shua_ka= []
    mycol = mydb['bus_card_record']
    myquery = {'riding_time':re.compile(time)}
    for x in mycol.find(myquery):
        shua_ka.append(x)
    shua_ka = pd.DataFrame(shua_ka)
    shua_ka_no = list(set(shua_ka['card_no']))
    mean = int(len(shua_ka_no)/16)
    mycol = mydb['stop_arrive_leave']
    
    a_up_down_list,bbb,not_pipei,onlyzaowan = multithreading(shua_ka_no,mean,chepai,jingweidu,shua_ka,mycol)
    a_up_down = {}
...
#            print('aaa')
    ...
    for i in a_up_down.keys():
        mydict_all = {'rate':a_up_down[i]['概率'],'cardId':str(i),'lat_long_morning':a_up_down[i]['经纬度_morning'],\
                      ...
                      'date':time,'cardType':3,'up_direct':a_up_down[i]['方向O'],'down_direct':a_up_down[i]['方向D']}
